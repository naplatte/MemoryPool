    ...
    static void warmup()
    {
        ...
        for (int i = 0; i < 1000; ++i)
        {
            for (size_t size : {8, 16, 32, 64, 128, 256, 512, 1024}) {
                void* p = MemoryPoolV2::allocate(size);
        ...
        {
            MemoryPoolV2::deallocate(ptr, size);  // 使用实际分配的大小进行释放
        }
        ...
        // 测试内存池
        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i)
            {
                ...
                if (i % 4 == 0)
                {
                    ...
                    if (!ptrs.empty())
                    {
                        MemoryPoolV2::deallocate(ptrs.back().first, ptrs.back().second);
                        ptrs.pop_back();
                    }
                }
            }
    ...
    static void testMultiThreaded()
    {
        ...
        auto threadFunc = [ALLOCS_PER_THREAD](bool useMemPool)
        {
            ...
            for (size_t i = 0; i < ALLOCS_PER_THREAD; ++i)
            {
                ...
                size_t size = SIZES[sizeIndex];
                void* ptr = useMemPool ? MemoryPoolV2::allocate(size)
                                       : new char[size];
                sizePtrs[sizeIndex].push_back({ptr, size});
                ...
                if (i % 100 == 0)  // 每100次分配
                {
                    ...
                    if (!ptrs.empty())
                    {
                        ...
                        for (size_t j = 0; j < releaseCount; ++j)
                        {
                            ...
                            if (useMemPool)
                            {
                                MemoryPoolV2::deallocate(ptrs[index].first, ptrs[index].second);
                            }
                            else
                            {
